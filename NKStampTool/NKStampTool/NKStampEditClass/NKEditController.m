...
@interface NKEditController () <StampListControllerDelegate,NKStampViewDelegate>
@end

/*========================================================
 ; NKEditController
 ========================================================*/
@implementation NKEditController{
...
-(UIImage *)createShareImage
{
    ...
    for(UIView *view in [canvasView subviews]){
        if([view isKindOfClass:[NKStampView class]]){
            NKStampView *stampView = (NKStampView *)view;
...
{
    NKStampView *layerStamp = [[NKStampView alloc] initWithImage:image];
                         ...
                         completion:^(BOOL finished){
                             ...
                             [UIView animateWithDuration:0.3
                                              ...
                                              completion:^(BOOL finished){
                                                  ...
                                                  selectStamp = nil;
                                                  NKStampView *layerStamp = [[canvasView subviews] lastObject];
                         ...
                         [UIView animateWithDuration:0.3
                                          animations:^{
                                              ...
                                              for(int i = 0; i < [[canvasView subviews] count]; i++) {
                                                  NKStampView *layerStamp = [[canvasView subviews] objectAtIndex:i];
...
-(void)layerUpButtonTaped:(UIButton *)button
{
    ...
    NKStampView *layerStamp = [subViews objectAtIndex:selectNum];
    [canvasView bringSubviewToFront:layerStamp];
    for(int i = selectNum+2; i < [subViews count]; i++) {
        NKStampView *layerStamp = [subViews objectAtIndex:i];
        [canvasView bringSubviewToFront:layerStamp];
    }
...
/*--------------------------------------------------------
 ...
 ;             in : (NKStampView *)stampView
 ;            out :
 --------------------------------------------------------*/
-(void)moveBeganStamp:(NKStampView *)stampView
{
    for(NKStampView *layerStamp in [canvasView subviews]) {
        //スタンプの選択の解除
